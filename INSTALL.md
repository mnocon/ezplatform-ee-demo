...
## Terms for future reference:
  * `/<root-dir>/`: The filesystem path where eZ Platform Enterprise Edition Demo is installed in.
...
## Prerequisite
  ...
  **Before you start**:
  ...
  - *Optional:* You can also setup Solr to be used by eZ Platform Enterprise Edition Demo and take note of the url it is accessible on

## Install

1. **Install/Extract eZ Platform Enterprise Edition Demo**<a name="install-1-extract"></a>:
    ...
    A. **Extract archive** (tar/zip)
      ...
      Extract the eZ Platform Enterprise Edition archive to a directory, then execute post install scripts:
    ...
    B. **Install via Composer**
     ...
     If you have a eZ Enterprise Subscription and [have setup composer authentication tokens](https://doc.ez.no/display/DEVELOPER/Using+Composer), you can get eZ Platform Enterprise Edition using composer with the following commands:
...
3. **Run installation command**<a name="install-4-db-setup"></a>:
    ...
    You may now complete the eZ Platform Enterprise Edition Demo installation with ezplatform:install command, example of use:
...
4. **Configure a VirtualHost**<a name="install-3-vhost"></a>:
    ...
    ```bash
    ./bin/vhost.sh --basedir=/var/www/ezplatform-ee-demo \
      --template-file=doc/apache2/vhost.template \
      --host-name=ezplatform-ee-demo \
      | sudo tee /etc/apache2/sites-enabled/ezplatform-ee-demo.conf > /dev/null
    ```
